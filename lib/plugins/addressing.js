        ...
        EN: (function() {
            ...
            return function(address) {
                var rawParts = splitAddress(address, regexSeparator, cleaners),
                    ...
                    // only attempt to extract street data if we have more than one part
                    // for instance, if we get parsed "ST GEORGE", we really don't want to interpret this as a street
    ...
    function extractStreetData(parts, streetRegexes) {
        ...
        for (var ii = parts.length; ii--; ) {
            for (var rgxIdx = 0; rgxIdx < streetRegexes.length; rgxIdx++) {
                ...
                if (streetRegexes[rgxIdx].test(parts[ii]) && ii > 0) {
